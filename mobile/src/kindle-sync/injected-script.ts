export function buildKindleSyncInjectedScript(): string {
  return `
    (function () {
      ...
      function inferMode(mode, asin) {
        if (mode === 'list' || mode === 'sync') {
          return mode;
        }
      ...
      function runNotebookFlow() {
        ...
        var mode = inferMode(getModeFromHash(), getASINFromURL());
        ...
        if (mode === 'sync') {
          ...
          waitForHighlights(function () {
            collectHighlightsWithProgress(asin, function (highlights) {
