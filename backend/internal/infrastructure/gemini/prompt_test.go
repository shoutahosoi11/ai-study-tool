...
func TestBuildBatchGeneratorPromptOmitsEmptyCustomInstruction(t *testing.T) {
	prompt, err := BuildBatchGeneratorPrompt([]domain.ExtractedPoint{{Point: "p"}},
		domain.QuestionTypeMultipleChoice, "   ")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// ガード文中の言及は許容し、ブロック（開始タグ+改行）が無いことだけ確認する。
	if strings.Contains(prompt, "<custom_instruction>\n") {
		t.Fatalf("blank custom instruction must not emit a block:\n%s", prompt)
	}
}
